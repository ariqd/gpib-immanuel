        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="bg-white border-b border-gray-200 p-4">
                    <form action="{{ @$image ? route('admin.carousel.update', @$image->id) : route('admin.carousel.store') }}"
                        autocomplete="off" method="POST" enctype="multipart/form-data">
                        @csrf
                        {{ @$image ? method_field('PUT') : '' }}
                            ...
                            <div class="mb-3 w-96">
                                ...
                                @if (@$image)
                                    <img src="{{ asset('uploads/carousel/image/' . @$image->carousel_image) }}"
                                        alt="{{ @$image->carousel_title }}" />
                                    <label for="formFile_image"
                                        class="block mt-4 mb-2 text-sm font-medium text-gray-900">{{ @$image ? 'Ganti' : '' }}
                                        Gambar
                                    </label>
                                @endif
